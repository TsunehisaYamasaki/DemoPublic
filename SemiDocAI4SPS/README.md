...
SharePoint Online の **folder1** にアップロードされた半導体関連ドキュメントを **Azure AI Document Intelligence** で自動テキスト化し、**Cosmos DB** に保存。さらに **Azure AI Search** でインデックス化し、**Azure OpenAI GPT-4o** を使った RAG (Retrieval-Augmented Generation) パターンで自然言語による質問応答を実現するサーバーレスシステムです。
